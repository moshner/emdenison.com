...
const Header = ({ scrollLink, children }: PropsWithChildren<Props>) => {
  ...
  return (
    <div className={`relative h-svh lg:h-96 bg-hero-voyager-20 bg-no-repeat bg-cover bg-right-3 lg:bg-top-3 max-w-screen-xl m-auto transition-[background] duration-[20000ms] hover:bg-left motion-reduce:transition-none motion-reduce:hover:bg-right-3 lg:rounded-b-2xl`}>
      <TopNav active="Books" menuColor="text-white" />
      <div className="backdrop-blur-xs h-full content-center lg:rounded-b-2xl">
        {children}
